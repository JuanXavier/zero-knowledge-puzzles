...
void QuadraticEquation_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void QuadraticEquation_2_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[3] = { 
IsZero_0_run,
IsEqual_1_run,
QuadraticEquation_2_run };
...
ctx->componentMemory[coffset].templateName = "QuadraticEquation";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_mul(&expaux[4],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 1]); // line circom 17
Fr_mul(&expaux[2],&signalValues[mySignalStart + 2],&expaux[4]); // line circom 17
Fr_mul(&expaux[3],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 1]); // line circom 17
Fr_add(&expaux[1],&expaux[2],&expaux[3]); // line circom 17
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 4]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
QuadraticEquation_2_create(1,0,ctx,"main",0);
QuadraticEquation_2_run(0,ctx);
}
