...
void Salt_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Salt_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_size_of_witness() {return 1985;}
...
ctx->componentMemory[coffset].templateName = "Salt";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 3;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
